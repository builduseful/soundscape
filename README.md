...
Media Session support is progressive: the app publishes metadata, registers only the supported play/pause/previous/next actions, keeps `playbackState` synchronized from real media events and Web Audio context state changes, and publishes the decoded loop-cycle position from the current `AudioBuffer`. The Audio Session API is also treated as a progressive enhancement and is set to `playback` when available.
